...
//    HCNetSDK INSTANCE = (HCNetSDK) Native.loadLibrary("C:\\software\\server\\lib\\HCNetSDK", HCNetSDK.class);
    ...
    class NET_DVR_FACE_PARAM_CTRL extends Structure {
        public int dwSize;
    ...
    class NET_DVR_FACE_PARAM_BYREADER extends Structure {
    ...
    class NET_DVR_FACE_PARAM_BYCARD extends Structure {
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN]; //人脸关联的卡号
     ...
     */
    boolean NET_DVR_SetLogToFile(int bLogEnable, byte[] strLogDir, boolean bAutoDel);
    ...
    boolean NET_DVR_SetAlarmDeviceUser(NativeLong lUserID, int lUserIndex, Pointer lpDeviceUser);

    //public boolean NET_DVR_GetAlarmDeviceUser(int lUserID, int lUserIndex, ref NET_DVR_ALARM_DEVICE_USER lpDeviceUser);
    boolean NET_DVR_GetAlarmDeviceUser(NativeLong lUserID, int lUserIndex, Pointer lpDeviceUser);
...
//    public boolean NET_DVR_SetDVRMessageCallBack_V31(MSGCallBack_V31 fMessageCallBack, int pUser);
    ...
    boolean NET_DVR_STDXMLConfig(int lUserID, int lpInputParam, int lpOutputParam);
    ...
    /*******************Global Error Code**********************/
    int NET_DVR_NOERROR = 0; //No Error
    int NET_DVR_PASSWORD_ERROR = 1;//Username or Password error
    int NET_DVR_NOENOUGHPRI = 2;//Don't have enough authority
    int NET_DVR_NOINIT = 3;//have not Initialized
    int NET_DVR_CHANNEL_ERROR = 4;//Channel number error
    int NET_DVR_OVER_MAXLINK = 5;//Number of clients connecting to DVR beyonds the Maximum
    int NET_DVR_VERSIONNOMATCH = 6;//Version is not matched
    int NET_DVR_NETWORK_FAIL_CONNECT = 7;//Connect to server failed
    int NET_DVR_NETWORK_SEND_ERROR = 8;//Send data to server failed
    int NET_DVR_NETWORK_RECV_ERROR = 9;//Receive data from server failed
    int NET_DVR_NETWORK_RECV_TIMEOUT = 10;//Receive data from server timeout
    int NET_DVR_NETWORK_ERRORDATA = 11;//Transferred data has error
    int NET_DVR_ORDER_ERROR = 12;//Wrong Sequence of invoking API
    int NET_DVR_OPERNOPERMIT = 13;//No such authority.
    int NET_DVR_COMMANDTIMEOUT = 14;//Execute command timeout
    int NET_DVR_ERRORSERIALPORT = 15;//Serial port number error
    int NET_DVR_ERRORALARMPORT = 16;//Alarm port error
    int NET_DVR_PARAMETER_ERROR = 17;//Parameters error
    int NET_DVR_CHAN_EXCEPTION = 18;//Server channel in error status
    int NET_DVR_NODISK = 19;//No hard disk
    int NET_DVR_ERRORDISKNUM = 20;//Hard disk number error
    int NET_DVR_DISK_FULL = 21;//Server's hard disk is full
    int NET_DVR_DISK_ERROR = 22;//Server's hard disk error
    int NET_DVR_NOSUPPORT = 23;//Server doesn't support
    int NET_DVR_BUSY = 24;//Server is busy
    int NET_DVR_MODIFY_FAIL = 25;//Server modification failed
    int NET_DVR_PASSWORD_FORMAT_ERROR = 26;///Input format of Password error
    int NET_DVR_DISK_FORMATING = 27;//Hard disk is formating,  cannot execute.
    int NET_DVR_DVRNORESOURCE = 28;//DVR don't have enough resource
    int NET_DVR_DVROPRATEFAILED = 29;//DVR Operation failed
    int NET_DVR_OPENHOSTSOUND_FAIL = 30;//Open PC audio failed
    int NET_DVR_DVRVOICEOPENED = 31;///Server's talk channel is occupied
    int NET_DVR_TIMEINPUTERROR = 32;//Time input is not correct
    int NET_DVR_NOSPECFILE = 33;//Can't playback the file that does not exist in Server
    int NET_DVR_CREATEFILE_ERROR = 34;//Create file error
    int NET_DVR_FILEOPENFAIL = 35;///Open file error
    int NET_DVR_OPERNOTFINISH = 36; //The previous operation is not finished yet
    int NET_DVR_GETPLAYTIMEFAIL = 37;//Get current playing time error
    int NET_DVR_PLAYFAIL = 38;//Playback error
    int NET_DVR_FILEFORMAT_ERROR = 39;//Wrong file format
    int NET_DVR_DIR_ERROR = 40;//Wrong directory
    int NET_DVR_ALLOC_RESOURCE_ERROR = 41;//Assign resource error
    int NET_DVR_AUDIO_MODE_ERROR = 42;//Audio card mode error
    int NET_DVR_NOENOUGH_BUF = 43;///Buffer is too small
    int NET_DVR_CREATESOCKET_ERROR = 44;//Create SOCKET error
    int NET_DVR_SETSOCKET_ERROR = 45;//Setup SOCKET error
    int NET_DVR_MAX_NUM = 46;//Reach the maximum number
    int NET_DVR_USERNOTEXIST = 47;//User does not exist
    int NET_DVR_WRITEFLASHERROR = 48;//Write to FLASH error
    int NET_DVR_UPGRADEFAIL = 49;//DVR update failed
    int NET_DVR_CARDHAVEINIT = 50;//Decoding Card has been initialized already
    int NET_DVR_PLAYERFAILED = 51;//Invoke API of player library error
    int NET_DVR_MAX_USERNUM = 52;//Reach the maximum number of Users
    int NET_DVR_GETLOCALIPANDMACFAIL = 53;//Failed to get Client software's IP or MAC address
    int NET_DVR_NOENCODEING = 54;//No encoding on this channel
    int NET_DVR_IPMISMATCH = 55;//IP address is not matched
    int NET_DVR_MACMISMATCH = 56;//MAC address is not matched

    int NET_DVR_USER_LOCKED = 153;
    /*******************END**********************/

    int NET_DVR_DEV_ADDRESS_MAX_LEN = 129; //device address max length
    int NET_DVR_LOGIN_USERNAME_MAX_LEN = 64;   //login username max length
    int NET_DVR_LOGIN_PASSWD_MAX_LEN = 64; //login password max length
    int SERIALNO_LEN = 48; //serial number length
    int STREAM_ID_LEN = 32;
    int MAX_AUDIO_V40 = 8;
    int LOG_INFO_LEN = 11840; // log append information

    int MAX_NAMELEN = 16;        //DVR's local Username
    int MAX_DOMAIN_NAME = 64;  // max domain name length
    int MAX_ETHERNET = 2;      // device
    int NAME_LEN = 32;// name length
    int PASSWD_LEN = 16;//password length
    int MAX_RIGHT = 32;        //Authority permitted by Device (1- 12 for local authority,  13- 32 for remote authority)
    int MACADDR_LEN = 6;//mac adress length
    int DEV_TYPE_NAME_LEN = 24;

    int MAX_ANALOG_CHANNUM = 32;      //32 analog channels in total
    int MAX_IP_CHANNEL = 32;      //9000 DVR can connect 32 IP channels
    int MAX_CHANNUM_V30 = (MAX_ANALOG_CHANNUM + MAX_IP_CHANNEL);   //64
    int MAX_CHANNUM_V40 = 512;
    int MAX_IP_DEVICE_V40 = 64;      //Maximum number of IP devices that can be added, the value is 64, including IVMS-2000
    int DEV_ID_LEN = 32;

    int MAX_IP_DEVICE = 32;//9000 DVR can connect 32 IP devices
    int MAX_IP_ALARMIN_V40 = 4096;//Maximum number of alarm input channels that can be added
    int MAX_IP_ALARMOUT_V40 = 4096;//Maximum number of alarm output channels that can be added
    int MAX_IP_ALARMIN = 128;//Maximum number of alarm input channels that can be added
    int MAX_IP_ALARMOUT = 64;//Maximum number of alarm output channels that can be added
    int URL_LEN = 240;   //URL length

    int ACS_ABILITY = 0x801; //acs ability

    int NET_DVR_CLEAR_ACS_PARAM = 2118;    //clear acs host parameters

    //region acs event upload

    int COMM_ALARM_ACS = 0x5002; //access card alarm
    ...
    //    boolean NET_DVR_StopPlayBackSave(NativeLong lPlayHandle);//停止播放
    boolean NET_DVR_StopPlayBack(NativeLong lPlayHandle);//停止播放
    ...
    int MAJOR_ALARM = 0x1;
    // Hypo- Type
    int MINOR_ALARMIN_SHORT_CIRCUIT = 0x400; // region short circuit
    int MINOR_ALARMIN_BROKEN_CIRCUIT = 0x401; // region broken circuit
    int MINOR_ALARMIN_EXCEPTION = 0x402; // region exception
    int MINOR_ALARMIN_RESUME = 0x403; // region resume
    int MINOR_HOST_DESMANTLE_ALARM = 0x404; // host desmantle alarm
    int MINOR_HOST_DESMANTLE_RESUME = 0x405; //  host desmantle resume
    int MINOR_CARD_READER_DESMANTLE_ALARM = 0x406; // card reader desmantle alarm
    int MINOR_CARD_READER_DESMANTLE_RESUME = 0x407; // card reader desmantle resume
    int MINOR_CASE_SENSOR_ALARM = 0x408; // case sensor alarm
    int MINOR_CASE_SENSOR_RESUME = 0x409; // case sensor resume
    int MINOR_STRESS_ALARM = 0x40a; // stress alarm
    int MINOR_OFFLINE_ECENT_NEARLY_FULL = 0x40b; // offline ecent nearly full
    int MINOR_CARD_MAX_AUTHENTICATE_FAIL = 0x40c; // card max authenticate fall
    int MINOR_SD_CARD_FULL = 0x40d; // SD card is full
    int MINOR_LINKAGE_CAPTURE_PIC = 0x40e; // lingage capture picture
    int MINOR_SECURITY_MODULE_DESMANTLE_ALARM = 0x40f;  //Door control security module desmantle alarm
    int MINOR_SECURITY_MODULE_DESMANTLE_RESUME = 0x410;  //Door control security module desmantle resume
    int MINOR_POS_START_ALARM = 0x411; // POS Start
    int MINOR_POS_END_ALARM = 0x412; // POS end
    int MINOR_FACE_IMAGE_QUALITY_LOW = 0x413; // face image quality low
    int MINOR_FINGE_RPRINT_QUALITY_LOW = 0x414; // finger print quality low
    int MINOR_FIRE_IMPORT_SHORT_CIRCUIT = 0x415; // Fire import short circuit
    int MINOR_FIRE_IMPORT_BROKEN_CIRCUIT = 0x416; // Fire import broken circuit
    int MINOR_FIRE_IMPORT_RESUME = 0x417; // Fire import resume
    int MINOR_FIRE_BUTTON_TRIGGER = 0x418; // fire button trigger
    int MINOR_FIRE_BUTTON_RESUME = 0x419; // fire button resume
    int MINOR_MAINTENANCE_BUTTON_TRIGGER = 0x41a; // maintenance button trigger
    int MINOR_MAINTENANCE_BUTTON_RESUME = 0x41b; // maintenance button resume
    int MINOR_EMERGENCY_BUTTON_TRIGGER = 0x41c; // emergency button trigger
    int MINOR_EMERGENCY_BUTTON_RESUME = 0x41d; // emergency button resume
    int MINOR_DISTRACT_CONTROLLER_ALARM = 0x41e; // distract controller alarm
    int MINOR_DISTRACT_CONTROLLER_RESUME = 0x41f; // distract controller resume
    ...
    int MINOR_NET_BROKEN = 0x27; // Network disconnected
    int MINOR_RS485_DEVICE_ABNORMAL = 0x3a; // RS485 connect status exception
    int MINOR_RS485_DEVICE_REVERT = 0x3b; // RS485 connect status exception recovery

    int MINOR_DEV_POWER_ON = 0x400; // device power on
    int MINOR_DEV_POWER_OFF = 0x401; // device power off
    int MINOR_WATCH_DOG_RESET = 0x402; // watch dog reset
    int MINOR_LOW_BATTERY = 0x403; // low battery
    int MINOR_BATTERY_RESUME = 0x404; // battery resume
    int MINOR_AC_OFF = 0x405; // AC off
    int MINOR_AC_RESUME = 0x406; // AC resume
    int MINOR_NET_RESUME = 0x407; // Net resume
    int MINOR_FLASH_ABNORMAL = 0x408; // FLASH abnormal
    int MINOR_CARD_READER_OFFLINE = 0x409; // card reader offline
    int MINOR_CARD_READER_RESUME = 0x40a; // card reader resume
    int MINOR_INDICATOR_LIGHT_OFF = 0x40b; // Indicator Light Off
    int MINOR_INDICATOR_LIGHT_RESUME = 0x40c; // Indicator Light Resume
    int MINOR_CHANNEL_CONTROLLER_OFF = 0x40d; // channel controller off
    int MINOR_CHANNEL_CONTROLLER_RESUME = 0x40e; // channel controller resume
    int MINOR_SECURITY_MODULE_OFF = 0x40f; // Door control security module off
    int MINOR_SECURITY_MODULE_RESUME = 0x410; // Door control security module resume
    int MINOR_BATTERY_ELECTRIC_LOW = 0x411; // battery electric low
    int MINOR_BATTERY_ELECTRIC_RESUME = 0x412; // battery electric resume
    int MINOR_LOCAL_CONTROL_NET_BROKEN = 0x413; // Local control net broken
    int MINOR_LOCAL_CONTROL_NET_RSUME = 0x414; // Local control net resume
    int MINOR_MASTER_RS485_LOOPNODE_BROKEN = 0x415; // Master RS485 loop node broken
    int MINOR_MASTER_RS485_LOOPNODE_RESUME = 0x416; // Master RS485 loop node resume
    int MINOR_LOCAL_CONTROL_OFFLINE = 0x417; // Local control offline
    int MINOR_LOCAL_CONTROL_RESUME = 0x418; // Local control resume
    int MINOR_LOCAL_DOWNSIDE_RS485_LOOPNODE_BROKEN = 0x419; // Local downside RS485 loop node broken
    int MINOR_LOCAL_DOWNSIDE_RS485_LOOPNODE_RESUME = 0x41a; // Local downside RS485 loop node resume
    int MINOR_DISTRACT_CONTROLLER_ONLINE = 0x41b; // distract controller online
    int MINOR_DISTRACT_CONTROLLER_OFFLINE = 0x41c; // distract controller offline
    int MINOR_ID_CARD_READER_NOT_CONNECT = 0x41d; // Id card reader not connected(intelligent dedicated)
    int MINOR_ID_CARD_READER_RESUME = 0x41e; //Id card reader connection restored(intelligent dedicated)
    int MINOR_FINGER_PRINT_MODULE_NOT_CONNECT = 0x41f; // fingerprint module is not connected(intelligent dedicated)
    int MINOR_FINGER_PRINT_MODULE_RESUME = 0x420; // The fingerprint module connection restored(intelligent dedicated)
    int MINOR_CAMERA_NOT_CONNECT = 0x421; // Camera not connected
    int MINOR_CAMERA_RESUME = 0x422; // Camera connection restored
    int MINOR_COM_NOT_CONNECT = 0x423; // COM not connected
    int MINOR_COM_RESUME = 0x424;// COM connection restored
    int MINOR_DEVICE_NOT_AUTHORIZE = 0x425; // device are not authorized
    int MINOR_PEOPLE_AND_ID_CARD_DEVICE_ONLINE = 0x426; // people and ID card device online
    int MINOR_PEOPLE_AND_ID_CARD_DEVICE_OFFLINE = 0x427;// people and ID card device offline
    int MINOR_LOCAL_LOGIN_LOCK = 0x428; // local login lock
    int MINOR_LOCAL_LOGIN_UNLOCK = 0x429; //local login unlock
    ...
    // Hypo- Type
    int MINOR_LOCAL_UPGRADE = 0x5a; // Upgrade  (local)
    int MINOR_REMOTE_LOGIN = 0x70; // Login  (remote)
    int MINOR_REMOTE_LOGOUT = 0x71; // Logout   (remote)
    int MINOR_REMOTE_ARM = 0x79; // On guard   (remote)
    int MINOR_REMOTE_DISARM = 0x7a; // Disarm   (remote)
    int MINOR_REMOTE_REBOOT = 0x7b; // Reboot   (remote)
    int MINOR_REMOTE_UPGRADE = 0x7e; // upgrade  (remote)
    int MINOR_REMOTE_CFGFILE_OUTPUT = 0x86; // Export Configuration   (remote)
    int MINOR_REMOTE_CFGFILE_INTPUT = 0x87; // Import Configuration  (remote)
    int MINOR_REMOTE_ALARMOUT_OPEN_MAN = 0xd6; // remote mamual open alarmout
    int MINOR_REMOTE_ALARMOUT_CLOSE_MAN = 0xd7; // remote mamual close alarmout

    int MINOR_REMOTE_OPEN_DOOR = 0x400; // remote open door
    int MINOR_REMOTE_CLOSE_DOOR = 0x401; // remote close door (controlled)
    int MINOR_REMOTE_ALWAYS_OPEN = 0x402; // remote always open door (free)
    int MINOR_REMOTE_ALWAYS_CLOSE = 0x403; // remote always close door (forbiden)
    int MINOR_REMOTE_CHECK_TIME = 0x404; // remote check time
    int MINOR_NTP_CHECK_TIME = 0x405; // ntp check time
    int MINOR_REMOTE_CLEAR_CARD = 0x406; // remote clear card
    int MINOR_REMOTE_RESTORE_CFG = 0x407; // remote restore configure
    int MINOR_ALARMIN_ARM = 0x408; // alarm in arm
    int MINOR_ALARMIN_DISARM = 0x409; // alarm in disarm
    int MINOR_LOCAL_RESTORE_CFG = 0x40a; // local configure restore
    int MINOR_REMOTE_CAPTURE_PIC = 0x40b; // remote capture picture
    int MINOR_MOD_NET_REPORT_CFG = 0x40c; // modify net report cfg
    int MINOR_MOD_GPRS_REPORT_PARAM = 0x40d; // modify GPRS report param
    int MINOR_MOD_REPORT_GROUP_PARAM = 0x40e; // modify report group param
    int MINOR_UNLOCK_PASSWORD_OPEN_DOOR = 0x40f; // unlock password open door
    int MINOR_AUTO_RENUMBER = 0x410; // auto renumber
    int MINOR_AUTO_COMPLEMENT_NUMBER = 0x411; // auto complement number
    int MINOR_NORMAL_CFGFILE_INPUT = 0x412; // normal cfg file input
    int MINOR_NORMAL_CFGFILE_OUTTPUT = 0x413; // normal cfg file output
    int MINOR_CARD_RIGHT_INPUT = 0x414; // card right input
    int MINOR_CARD_RIGHT_OUTTPUT = 0x415; // card right output
    int MINOR_LOCAL_USB_UPGRADE = 0x416; // local USB upgrade
    int MINOR_REMOTE_VISITOR_CALL_LADDER = 0x417; // visitor call ladder
    int MINOR_REMOTE_HOUSEHOLD_CALL_LADDER = 0x418; // household call ladder

    /* Additional Log Info*/
    // Main Type
    int MAJOR_EVENT = 0x5;/*event*/
    // Hypo- Type
    int MINOR_LEGAL_CARD_PASS = 0x01; // legal card pass
    int MINOR_CARD_AND_PSW_PASS = 0x02; // swipe and password pass
    int MINOR_CARD_AND_PSW_FAIL = 0x03; // swipe and password fail
    int MINOR_CARD_AND_PSW_TIMEOUT = 0x04; // swipe and password timeout
    int MINOR_CARD_AND_PSW_OVER_TIME = 0x05; // swipe and password over time
    int MINOR_CARD_NO_RIGHT = 0x06; // card no right
    int MINOR_CARD_INVALID_PERIOD = 0x07; // invalid period
    int MINOR_CARD_OUT_OF_DATE = 0x08; // card out of date
    int MINOR_INVALID_CARD = 0x09; // invalid card
    int MINOR_ANTI_SNEAK_FAIL = 0x0a; // anti sneak fail
    int MINOR_INTERLOCK_DOOR_NOT_CLOSE = 0x0b; // interlock door doesn't close
    int MINOR_NOT_BELONG_MULTI_GROUP = 0x0c; // card no belong multi group
    int MINOR_INVALID_MULTI_VERIFY_PERIOD = 0x0d; // invalid multi verify period
    int MINOR_MULTI_VERIFY_SUPER_RIGHT_FAIL = 0x0e; // have no super right in multi verify mode
    int MINOR_MULTI_VERIFY_REMOTE_RIGHT_FAIL = 0x0f; // have no remote right in multi verify mode
    int MINOR_MULTI_VERIFY_SUCCESS = 0x10; // success in multi verify mode
    int MINOR_LEADER_CARD_OPEN_BEGIN = 0x11; // leader card begin to open
    int MINOR_LEADER_CARD_OPEN_END = 0x12; // leader card end to open
    int MINOR_ALWAYS_OPEN_BEGIN = 0x13; // always open begin
    int MINOR_ALWAYS_OPEN_END = 0x14; // always open end
    int MINOR_LOCK_OPEN = 0x15; // lock open
    int MINOR_LOCK_CLOSE = 0x16; // lock close
    int MINOR_DOOR_BUTTON_PRESS = 0x17; // press door open button
    int MINOR_DOOR_BUTTON_RELEASE = 0x18; // release door open button
    int MINOR_DOOR_OPEN_NORMAL = 0x19; // door open normal
    int MINOR_DOOR_CLOSE_NORMAL = 0x1a; // door close normal
    int MINOR_DOOR_OPEN_ABNORMAL = 0x1b; // open door abnormal
    int MINOR_DOOR_OPEN_TIMEOUT = 0x1c; // open door timeout
    int MINOR_ALARMOUT_ON = 0x1d; // alarm out turn on
    int MINOR_ALARMOUT_OFF = 0x1e; // alarm out turn off
    int MINOR_ALWAYS_CLOSE_BEGIN = 0x1f; // always close begin
    int MINOR_ALWAYS_CLOSE_END = 0x20; // always close end
    int MINOR_MULTI_VERIFY_NEED_REMOTE_OPEN = 0x21; // need remote open in multi verify mode
    int MINOR_MULTI_VERIFY_SUPERPASSWD_VERIFY_SUCCESS = 0x22; // superpasswd verify success in multi verify mode
    int MINOR_MULTI_VERIFY_REPEAT_VERIFY = 0x23; // repeat verify in multi verify mode
    int MINOR_MULTI_VERIFY_TIMEOUT = 0x24; // timeout in multi verify mode
    int MINOR_DOORBELL_RINGING = 0x25; // doorbell ringing
    int MINOR_FINGERPRINT_COMPARE_PASS = 0x26; // fingerprint compare pass
    int MINOR_FINGERPRINT_COMPARE_FAIL = 0x27; // fingerprint compare fail
    int MINOR_CARD_FINGERPRINT_VERIFY_PASS = 0x28; // card and fingerprint verify pass
    int MINOR_CARD_FINGERPRINT_VERIFY_FAIL = 0x29; // card and fingerprint verify fail
    int MINOR_CARD_FINGERPRINT_VERIFY_TIMEOUT = 0x2a; // card and fingerprint verify timeout
    int MINOR_CARD_FINGERPRINT_PASSWD_VERIFY_PASS = 0x2b; // card and fingerprint and passwd verify pass
    int MINOR_CARD_FINGERPRINT_PASSWD_VERIFY_FAIL = 0x2c; // card and fingerprint and passwd verify fail
    int MINOR_CARD_FINGERPRINT_PASSWD_VERIFY_TIMEOUT = 0x2d; // card and fingerprint and passwd verify timeout
    int MINOR_FINGERPRINT_PASSWD_VERIFY_PASS = 0x2e; // fingerprint and passwd verify pass
    int MINOR_FINGERPRINT_PASSWD_VERIFY_FAIL = 0x2f; // fingerprint and passwd verify fail
    int MINOR_FINGERPRINT_PASSWD_VERIFY_TIMEOUT = 0x30; // fingerprint and passwd verify timeout
    int MINOR_FINGERPRINT_INEXISTENCE = 0x31; // fingerprint inexistence
    int MINOR_CARD_PLATFORM_VERIFY = 0x32; // card platform verify
    int MINOR_CALL_CENTER = 0x33; // call center
    int MINOR_FIRE_RELAY_TURN_ON_DOOR_ALWAYS_OPEN = 0x34; // fire relay turn on door always open
    int MINOR_FIRE_RELAY_RECOVER_DOOR_RECOVER_NORMAL = 0x35; // fire relay recover door recover normal
    int MINOR_FACE_AND_FP_VERIFY_PASS = 0x36; // face and finger print verify pass
    int MINOR_FACE_AND_FP_VERIFY_FAIL = 0x37; // face and finger print verify fail
    int MINOR_FACE_AND_FP_VERIFY_TIMEOUT = 0x38; // face and finger print verify timeout
    int MINOR_FACE_AND_PW_VERIFY_PASS = 0x39; // face and password verify pass
    int MINOR_FACE_AND_PW_VERIFY_FAIL = 0x3a; // face and password verify fail
    int MINOR_FACE_AND_PW_VERIFY_TIMEOUT = 0x3b; // face and password verify timeout
    int MINOR_FACE_AND_CARD_VERIFY_PASS = 0x3c; // face and card verify pass
    int MINOR_FACE_AND_CARD_VERIFY_FAIL = 0x3d; // face and card verify fail
    int MINOR_FACE_AND_CARD_VERIFY_TIMEOUT = 0x3e; // face and card verify timeout
    int MINOR_FACE_AND_PW_AND_FP_VERIFY_PASS = 0x3f; // face and password and finger print verify pass
    int MINOR_FACE_AND_PW_AND_FP_VERIFY_FAIL = 0x40; // face and password and finger print verify fail
    int MINOR_FACE_AND_PW_AND_FP_VERIFY_TIMEOUT = 0x41; // face and password and finger print verify timeout
    int MINOR_FACE_CARD_AND_FP_VERIFY_PASS = 0x42; // face and card and finger print verify pass
    int MINOR_FACE_CARD_AND_FP_VERIFY_FAIL = 0x43; // face and card and finger print verify fail
    int MINOR_FACE_CARD_AND_FP_VERIFY_TIMEOUT = 0x44; // face and card and finger print verify timeout
    int MINOR_EMPLOYEENO_AND_FP_VERIFY_PASS = 0x45; // employee and finger print verify pass
    int MINOR_EMPLOYEENO_AND_FP_VERIFY_FAIL = 0x46; // employee and finger print verify fail
    int MINOR_EMPLOYEENO_AND_FP_VERIFY_TIMEOUT = 0x47; // employee and finger print verify timeout
    int MINOR_EMPLOYEENO_AND_FP_AND_PW_VERIFY_PASS = 0x48; // employee and finger print and password verify pass
    int MINOR_EMPLOYEENO_AND_FP_AND_PW_VERIFY_FAIL = 0x49; // employee and finger print and password verify fail
    int MINOR_EMPLOYEENO_AND_FP_AND_PW_VERIFY_TIMEOUT = 0x4a; // employee and finger print and password verify timeout
    int MINOR_FACE_VERIFY_PASS = 0x4b; // face verify pass
    int MINOR_FACE_VERIFY_FAIL = 0x4c; // face verify fail
    int MINOR_EMPLOYEENO_AND_FACE_VERIFY_PASS = 0x4d; // employee no and face verify pass
    int MINOR_EMPLOYEENO_AND_FACE_VERIFY_FAIL = 0x4e; // employee no and face verify fail
    int MINOR_EMPLOYEENO_AND_FACE_VERIFY_TIMEOUT = 0x4f; // employee no and face verify time out
    int MINOR_FACE_RECOGNIZE_FAIL = 0x50; // face recognize fail
    int MINOR_FIRSTCARD_AUTHORIZE_BEGIN = 0x51; // first card authorize begin
    int MINOR_FIRSTCARD_AUTHORIZE_END = 0x52; // first card authorize end
    int MINOR_DOORLOCK_INPUT_SHORT_CIRCUIT = 0x53; // door lock input short circuit
    int MINOR_DOORLOCK_INPUT_BROKEN_CIRCUIT = 0x54; // door lock input broken circuit
    int MINOR_DOORLOCK_INPUT_EXCEPTION = 0x55; // door lock input exception
    int MINOR_DOORCONTACT_INPUT_SHORT_CIRCUIT = 0x56; // door contact input short circuit
    int MINOR_DOORCONTACT_INPUT_BROKEN_CIRCUIT = 0x57; // door contact input broken circuit
    int MINOR_DOORCONTACT_INPUT_EXCEPTION = 0x58; // door contact input exception
    int MINOR_OPENBUTTON_INPUT_SHORT_CIRCUIT = 0x59; // open button input short circuit
    int MINOR_OPENBUTTON_INPUT_BROKEN_CIRCUIT = 0x5a; // open button input broken circuit
    int MINOR_OPENBUTTON_INPUT_EXCEPTION = 0x5b; // open button input exception
    int MINOR_DOORLOCK_OPEN_EXCEPTION = 0x5c; // door lock open exception
    int MINOR_DOORLOCK_OPEN_TIMEOUT = 0x5d; // door lock open timeout
    int MINOR_FIRSTCARD_OPEN_WITHOUT_AUTHORIZE = 0x5e; // first card open without authorize
    int MINOR_CALL_LADDER_RELAY_BREAK = 0x5f; // call ladder relay break
    int MINOR_CALL_LADDER_RELAY_CLOSE = 0x60; // call ladder relay close
    int MINOR_AUTO_KEY_RELAY_BREAK = 0x61; // auto key relay break
    int MINOR_AUTO_KEY_RELAY_CLOSE = 0x62; // auto key relay close
    int MINOR_KEY_CONTROL_RELAY_BREAK = 0x63; // key control relay break
    int MINOR_KEY_CONTROL_RELAY_CLOSE = 0x64; // key control relay close
    int MINOR_EMPLOYEENO_AND_PW_PASS = 0x65; // minor employee no and password pass
    int MINOR_EMPLOYEENO_AND_PW_FAIL = 0x66; // minor employee no and password fail
    int MINOR_EMPLOYEENO_AND_PW_TIMEOUT = 0x67; // minor employee no and password timeout
    int MINOR_HUMAN_DETECT_FAIL = 0x68; // human detect fail
    int MINOR_PEOPLE_AND_ID_CARD_COMPARE_PASS = 0x69; // the comparison with people and id card success
    int MINOR_PEOPLE_AND_ID_CARD_COMPARE_FAIL = 0x70; // the comparison with people and id card failed
    int MINOR_CERTIFICATE_BLACK_LIST = 0x71; // black list
    int MINOR_LEGAL_MESSAGE = 0x72; // legal message
    int MINOR_ILLEGAL_MESSAGE = 0x73; // illegal messag
    int MINOR_MAC_DETECT = 0x74; // mac detect

    //region card parameters configuration
    int CARD_PARAM_CARD_VALID = 0x00000001;  //card valid parameter
    int CARD_PARAM_VALID = 0x00000002;  //valid period parameter
    int CARD_PARAM_CARD_TYPE = 0x00000004;  //card type parameter
    int CARD_PARAM_DOOR_RIGHT = 0x00000008;  //door right parameter
    int CARD_PARAM_LEADER_CARD = 0x00000010;  //leader card parameter
    int CARD_PARAM_SWIPE_NUM = 0x00000020;  //max swipe time parameter
    int CARD_PARAM_GROUP = 0x00000040;  //belong group parameter
    int CARD_PARAM_PASSWORD = 0x00000080;  //card password parameter
    int CARD_PARAM_RIGHT_PLAN = 0x00000100;  //card right plan parameter
    int CARD_PARAM_SWIPED_NUM = 0x00000200;  //has swiped card time parameter
    int CARD_PARAM_EMPLOYEE_NO = 0x00000400;  //employee no

    int ACS_CARD_NO_LEN = 32;  //access card No. len
    int MAX_DOOR_NUM_256 = 256; //max door num
    int MAX_GROUP_NUM_128 = 128; //The largest number of grou
    int CARD_PASSWORD_LEN = 8;   // card password len
    int MAX_CARD_RIGHT_PLAN_NUM = 4;   //max card right plan number
    int MAX_DOOR_CODE_LEN = 8; //room code length
    int MAX_LOCK_CODE_LEN = 8; //lock code length

    int MAX_CASE_SENSOR_NUM = 8;   //max case sensor number
    int MAX_CARD_READER_NUM_512 = 512; //max card reader num
    int MAX_ALARMHOST_ALARMIN_NUM = 512; //Max number of alarm host alarm input ports
    int MAX_ALARMHOST_ALARMOUT_NUM = 512; //Max number of alarm host alarm output ports

    int NET_DVR_GET_ACS_WORK_STATUS_V50 = 2180;   //Access door host working condition (V50)
    int NET_DVR_GET_CARD_CFG_V50 = 2178;    //Parameters to acquire new CARDS (V50)
    int NET_DVR_SET_CARD_CFG_V50 = 2179;    //Setting up the new parameters (V50)

    //region door parameters configuration
    int DOOR_NAME_LEN = 32;//door name len
    int STRESS_PASSWORD_LEN = 8;//stress password len
    int SUPER_PASSWORD_LEN = 8;//super password len
    int UNLOCK_PASSWORD_LEN = 8;
    int MAX_DOOR_NUM = 32;
    int MAX_GROUP_NUM = 32;
    int LOCAL_CONTROLLER_NAME_LEN = 32;

    int NET_DVR_GET_DOOR_CFG = 2108; //get door parameter
    int NET_DVR_SET_DOOR_CFG = 2109; //set door parameter

    //region group configuration

    int GROUP_NAME_LEN = 32;

    int NET_DVR_GET_GROUP_CFG = 2112;   //get group parameter
    int NET_DVR_SET_GROUP_CFG = 2113;    //set group parameter

    //region user parameters configuration

    int MAX_ALARMHOST_VIDEO_CHAN = 64;

    int NET_DVR_GET_DEVICECFG_V40 = 1100;//Get extended device parameters
    int NET_DVR_SET_DEVICECFG_V40 = 1101;//Set extended device parameters

    //region cardreader configuration

    int CARD_READER_DESCRIPTION = 32;       //card reader description

    int NET_DVR_GET_CARD_READER_CFG_V50 = 2505;      //get card reader configure v50
    int NET_DVR_SET_CARD_READER_CFG_V50 = 2506;      //set card reader configure v50

    //region fingerprint configuration

    int MAX_FINGER_PRINT_LEN = 768; //max finger print len
    int MAX_FINGER_PRINT_NUM = 10; //max finger print num
    int ERROR_MSG_LEN = 32;

    int NET_DVR_GET_FINGERPRINT_CFG = 2150;    //get fingerprint parameter
    int NET_DVR_SET_FINGERPRINT_CFG = 2151;    //set fingerprint parameter
    int NET_DVR_DEL_FINGERPRINT_CFG = 2152;    //delete fingerprint parameter
    ...
    int MAX_DAYS = 7; //The number of days in a week
    int MAX_TIMESEGMENT_V30 = 8; //Maximum number of time segments in 9000 DVR's guard schedule
    int HOLIDAY_GROUP_NAME_LEN = 32;  //holiday group name len
    int MAX_HOLIDAY_PLAN_NUM = 16;  //holiday max plan number
    int TEMPLATE_NAME_LEN = 32; //plan template name len
    int MAX_HOLIDAY_GROUP_NUM = 16;   //plan template max group number

    int NET_DVR_GET_WEEK_PLAN_CFG = 2100; //get door status week plan config
    int NET_DVR_SET_WEEK_PLAN_CFG = 2101; //set door status week plan config
    int NET_DVR_GET_DOOR_STATUS_HOLIDAY_PLAN = 2102; //get door status holiday week plan config
    int NET_DVR_SET_DOOR_STATUS_HOLIDAY_PLAN = 2103; //set door status holiday week plan config
    int NET_DVR_GET_DOOR_STATUS_HOLIDAY_GROUP = 2104; //get door holiday group parameter
    int NET_DVR_SET_DOOR_STATUS_HOLIDAY_GROUP = 2105; //set door holiday group parameter
    int NET_DVR_GET_DOOR_STATUS_PLAN_TEMPLATE = 2106; //get door status plan template parameter
    int NET_DVR_SET_DOOR_STATUS_PLAN_TEMPLATE = 2107; //set door status plan template parameter
    int NET_DVR_GET_VERIFY_WEEK_PLAN = 2124; //get reader card verfy week plan
    int NET_DVR_SET_VERIFY_WEEK_PLAN = 2125; //set reader card verfy week plan
    int NET_DVR_GET_CARD_RIGHT_WEEK_PLAN = 2126;  //get card right week plan
    int NET_DVR_SET_CARD_RIGHT_WEEK_PLAN = 2127; //set card right week plan
    int NET_DVR_GET_VERIFY_HOLIDAY_PLAN = 2128; //get card reader verify holiday plan
    int NET_DVR_SET_VERIFY_HOLIDAY_PLAN = 2129; //set card reader verify holiday plan
    int NET_DVR_GET_CARD_RIGHT_HOLIDAY_PLAN = 2130; //get card right holiday plan
    int NET_DVR_SET_CARD_RIGHT_HOLIDAY_PLAN = 2131; //set card right holiday plan
    int NET_DVR_GET_VERIFY_HOLIDAY_GROUP = 2132; //get card reader verify holiday group
    int NET_DVR_SET_VERIFY_HOLIDAY_GROUP = 2133; //set card reader verify holiday group
    int NET_DVR_GET_CARD_RIGHT_HOLIDAY_GROUP = 2134; //get card right holiday group
    int NET_DVR_SET_CARD_RIGHT_HOLIDAY_GROUP = 2135; //set card right holiday group
    int NET_DVR_GET_VERIFY_PLAN_TEMPLATE = 2136; //get card reader verify plan template
    int NET_DVR_SET_VERIFY_PLAN_TEMPLATE = 2137; //set card reader verify plan template
    int NET_DVR_GET_CARD_RIGHT_PLAN_TEMPLATE = 2138; //get card right plan template
    int NET_DVR_SET_CARD_RIGHT_PLAN_TEMPLATE = 2139; //set card right plan template
    // V50
    int NET_DVR_GET_CARD_RIGHT_WEEK_PLAN_V50 = 2304;  //Access card right V50 weeks plan parameters
    int NET_DVR_SET_CARD_RIGHT_WEEK_PLAN_V50 = 2305;  //Set card right V50 weeks plan parameters
    int NET_DVR_GET_CARD_RIGHT_HOLIDAY_PLAN_V50 = 2310;  //Access card right parameters V50 holiday plan
    int NET_DVR_SET_CARD_RIGHT_HOLIDAY_PLAN_V50 = 2311;  //Set card right parameters V50 holiday plan
    int NET_DVR_GET_CARD_RIGHT_HOLIDAY_GROUP_V50 = 2316; //Access card right parameters V50 holiday group
    int NET_DVR_SET_CARD_RIGHT_HOLIDAY_GROUP_V50 = 2317; //Set card right parameters V50 holiday group
    int NET_DVR_GET_CARD_RIGHT_PLAN_TEMPLATE_V50 = 2322; //Access card right parameters V50 plan template
    int NET_DVR_SET_CARD_RIGHT_PLAN_TEMPLATE_V50 = 2323; //Set card right parameters V50 plan template

    //region card reader verification mode and door status planning parameters configuration

    int NET_DVR_GET_DOOR_STATUS_PLAN = 2110; //get door status plan parameter
    int NET_DVR_SET_DOOR_STATUS_PLAN = 2111; //set door status plan parameter
    int NET_DVR_GET_CARD_READER_PLAN = 2142; //get card reader verify plan parameter
    int NET_DVR_SET_CARD_READER_PLAN = 2143; //get card reader verify plan parameter

    //region card number associated with the user information parameter configuration

    int NET_DVR_GET_CARD_USERINFO_CFG = 2163; //get card userinfo cfg
    int NET_DVR_SET_CARD_USERINFO_CFG = 2164; //set card userinfo cfg

    //region event card linkage

    int NET_DVR_GET_EVENT_CARD_LINKAGE_CFG_V50 = 2181; //get event card linkage cfg
    int NET_DVR_SET_EVENT_CARD_LINKAGE_CFG_V50 = 2182; //set event card linkage cfg

    //region net configuration

    int NET_DVR_GET_NETCFG_V30 = 1000;//Get network parameter configuration
    int NET_DVR_SET_NETCFG_V30 = 1001;//Set network parameter configuration
    int NET_DVR_GET_NETCFG_V50 = 1015;    //Get network parameter configuration (V50)
    int NET_DVR_SET_NETCFG_V50 = 1016;    //Set network parameter configuration (V50)
    ...
    class NET_DVR_DATE extends Structure //convert type short to short in Structure
    {
    ...
    class NET_DVR_SIMPLE_DAYTIME extends Structure {
    ...
    class NET_DVR_TIME_EX extends Structure //convert type short to short in Structure
    {
        public short wYear;
        public byte byMonth;
        public byte byDay;
    ...
    enum LONG_CFG_SEND_DATA_TYPE_ENUM {
        ...
        LONG_CFG_SEND_DATA_TYPE_ENUM(int value) {
            this.value = value;
        }
    }

    enum NET_SDK_GET_NEXT_STATUS {
        ...
        NET_SDK_GET_NEXT_STATUS(int value) {
            this.value = value;
        }
    }

    enum LONG_CFG_RECV_DATA_TYPE_ENUM {
        ENUM_DVR_ERROR_CODE(1), //Error code
        ENUM_MSC_RECV_DATA(2), //screen control data type
        ENUM_ACS_RECV_DATA(3); //ACS control data type
        private final int value;

        LONG_CFG_RECV_DATA_TYPE_ENUM(int value) {
            this.value = value;
        }
    }

    enum ACS_DEV_SUBEVENT_ENUM {
        ...
        ACS_DEV_SUBEVENT_ENUM(int value) {
            this.value = value;
        }
    }

    enum ACS_ALARM_SUBEVENT_ENUM {
        ...
        ACS_ALARM_SUBEVENT_ENUM(int value) {
            this.value = value;
        }
    }

    enum ACS_DOOR_SUBEVENT_ENUM {
        ...
        ACS_DOOR_SUBEVENT_ENUM(int value) {
            this.value = value;
        }
    }

    enum ACS_CARD_READER_SUBEVENT_ENUM {
        ...
        ACS_CARD_READER_SUBEVENT_ENUM(int value) {
            this.value = value;
        }
    }


    class NET_DVR_XML_CONFIG_INPUT extends Structure//convert type int to int in Structure
    {
    ...
    class NET_DVR_XML_CONFIG_OUTPUT extends Structure//convert type int to int in Structure
    {
    ...
    class NET_DVR_LOG_V30 extends Structure//convert type int to int, string to String in Structure
    {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("strLogTime", "dwMajorType", "dwMinorType", "sPanelUser", "sNetUser",
                    "struRemoteHostAddr", "dwParaType", "dwChannel", "dwDiskNumber", "dwAlarmInPort", "dwAlarmOutPort", "dwInfoLen", "sInfo");
        }
    }

    //  ACS event informations
    class NET_DVR_ACS_EVENT_INFO extends Structure//convert type int to int, short to short in Structure
    {
        public int dwSize;
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("dwSize", "byCardNo", "byCardType", "byWhiteListNo", "byReportChannel", "byCardReaderKind", "dwCardReaderNo", "dwDoorNo", "dwVerifyNo",
    ...
    class NET_DVR_ACS_ALARM_INFO extends Structure//convert type int to int in Structure
    {
        public int dwSize;
    ...
    //Alarm Device Infor
    class NET_DVR_ALARMER extends Structure//convert type int to int, short to short, string to String in Structure
    {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("byUserIDValid", "bySerialValid", "byVersionValid", "byDeviceNameValid", "byMacAddrValid", "byLinkPortValid", "byDeviceIPValid",
    ...
    class NET_DVR_SETUPALARM_PARAM extends Structure//convert type int to int, short to short in Structure
    {
        public int dwSize;
    ...
    class CARDRIGHTPLAN_WORD extends Structure {
        public short[] wRightPlan = new short[MAX_CARD_RIGHT_PLAN_NUM];
    }

    class NET_DVR_CARD_CFG_V50 extends Structure {
        public int dwSize;
    ...
    class NET_DVR_ACS_PARAM_TYPE extends Structure {
        public int dwSize;
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("dwSize", "dwParamType", "wLocalControllerID", "byRes");
        }
    }

    //region door parameters configuration

    class NET_DVR_DOOR_CFG extends Structure {
        public int dwSize;
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("dwSize", "byDoorName", "byMagneticType", "byOpenButtonType", "byOpenDuration", "byDisabledOpenDuration",
    ...
    class NET_DVR_GROUP_CFG extends Structure {
        public int dwSize;
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("dwSize", "byEnable", "byRes1", "struValidPeriodCfg", "byGroupName", "byRes2");
        }
    }

    //region user parameters configuration

    class NET_DVR_ALARM_DEVICE_USER extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("dwSize", "sUserName", "sPassword", "struUserIP", "byAMCAddr", "byUserType", "byAlarmOnRight", "byAlarmOffRight", "byBypassRight",
                    "byOtherRight", "byNetPreviewRight", "byNetRecordRight", "byNetPlaybackRight", "byNetPTZRight", "sOriginalPassword", "byRes2");
        }
    }

    class NET_DVR_FACE_PARAM_COND extends Structure {
        public int dwSize;
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN];    //人脸关联的卡号
    ...
    class NET_DVR_FACE_PARAM_CFG extends Structure {
        public int dwSize;
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN];    //人脸关联的卡号
    ...
    class NET_DVR_CARD_READER_CFG_V50 extends Structure {
        public int dwSize;
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("dwSize", "byEnable", "byCardReaderType", "byOkLedPolarity", "byErrorLedPolarity",
    ...
    class NET_DVR_FINGER_PRINT_CFG extends Structure {
        public int dwSize;
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN]; //card NO
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("dwSize", "byCardNo", "dwFingerPrLen", "byEnableCardReader", "byFingerPrID",
                    "byFingerType", "byRes1", "byFingerData", "byRes");
        }
    }

    class NET_DVR_FINGER_PRINT_STATUS extends Structure {
        public int dwSize;
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("dwSize", "byCardNo", "byCardReaderRecvStatus", "byFingerPrID", "byFingerType",
                    "byTotalStatus", "byRes1", "byErrorMsg", "dwCardReaderNo", "byRes");
        }
    }

    class NET_DVR_FINGER_PRINT_INFO_COND extends Structure {
        public int dwSize;
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN];
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("dwSize", "byCardNo", "byEnableCardReader", "dwFingerPrintNum", "byFingerPrintID", "byCallbackMode", "byRes1");
        }
    }

    class NET_DVR_FINGER_PRINT_BYCARD extends Structure {
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN];
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("byCardNo", "byEnableCardReader", "byFingerPrintID", "byRes1");
        }
    }

    class NET_DVR_FINGER_PRINT_BYREADER extends Structure {
    ...
    class NET_DVR_FINGER_PRINT_INFO_CTRL_BYCARD extends Structure {
        public int dwSize;
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("dwSize", "byMode", "byRes1", "struByCard", "byRes");
        }

    }

    class NET_DVR_FINGER_PRINT_INFO_CTRL_BYREADER extends Structure {
        public int dwSize;
        public byte byMode;          //delete mode,0-delete by card,1-delete by reader
        public byte[] byRes1 = new byte[3];
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("dwSize", "byMode", "byRes1", "struByReader", "byRes");
        }
    }

    //region plan configuration

    class NET_DVR_TIME_SEGMENT extends Structure {
    ...
    class NET_DVR_SINGLE_PLAN_SEGMENT extends Structure {
        public byte byEnable; //whether to enable, 1-enable, 0-disable
    ...
    //2-dimen array
    class arrayStruPlanCfg extends Structure {
    ...
    class NET_DVR_WEEK_PLAN_CFG extends Structure {
        public int dwSize;
    ...
    class NET_DVR_HOLIDAY_PLAN_CFG extends Structure {
        public int dwSize;
        public byte byEnable;  //whether to enable, 1-enable, 0-disable
        public byte[] byRes1 = new byte[3];
    ...
    class NET_DVR_HOLIDAY_GROUP_CFG extends Structure {
        public int dwSize;
        public byte byEnable; //whether to enable, 1-enable, 0-disable
        public byte[] byRes1 = new byte[3];
    ...
    class NET_DVR_PLAN_TEMPLATE extends Structure {
        public int dwSize;
        public byte byEnable; //whether to enable, 1-enable, 0-disable
        public byte[] byRes1 = new byte[3];
    ...
    class NET_DVR_HOLIDAY_PLAN_COND extends Structure {
        public int dwSize;
    ...
    class NET_DVR_WEEK_PLAN_COND extends Structure {
        public int dwSize;
    ...
    class NET_DVR_HOLIDAY_GROUP_COND extends Structure {
        public int dwSize;
    ...
    class NET_DVR_PLAN_TEMPLATE_COND extends Structure {
        public int dwSize;
    ...
    class NET_DVR_DOOR_STATUS_PLAN extends Structure {
        public int dwSize;
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("dwSize", "dwTemplateNo", "byRes");
        }
    }

    class NET_DVR_CARD_READER_PLAN extends Structure {
        public int dwSize;
    ...
    class NET_DVR_CARD_USER_INFO_CFG extends Structure {
        public int dwSize;
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("dwSize", "byUsername", "byRes2");
        }
    }
...
//bySupport5 &0x04support sence expend
    ...
    class NET_DVR_DEVICEINFO_V40 extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("struDeviceV30", "bySupportLock", "byRetryLoginTime", "byPasswordLevel", "byProxyType",
    ...
    class NET_DVR_DEVICECFG_V40 extends Structure {
        public int dwSize;
    ...
    interface FLoginResultCallBack extends StdCallCallback {
        int invoke(NativeLong lUserID, int dwResult, NET_DVR_DEVICEINFO_V30 lpDeviceinfo, Pointer pUser);
    }

    class NET_DVR_USER_LOGIN_INFO extends Structure /*Change string to byte []*/ {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("sDeviceAddress", "byUseTransport", "wPort", "sUserName", "sPassword", "cbLoginResult", "pUser",
    ...
    class NET_DVR_PREVIEWINFO extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("lChannel", "dwStreamType", "dwLinkMode", "hPlayWnd", "bBlocked", "bPassbackRecord",
    ...
    class NET_DVR_PPPOECFG extends Structure {
    ...
    class NET_DVR_NETCFG_V50 extends Structure {
        public int dwSize;
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("dwSize", "struEtherNet", "struRes1", "struAlarmHostIpAddr", "byRes2",
    ...
    class NET_DVR_IPDEVINFO_V31 extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("byEnable", "byProType", "byEnableQuickAdd", "byRes1", "sUserName", "sPassword", "byDomain",
                    "struIP", "wDVRPort", "szDeviceID", "byRes2");
        }
    }

    //region event card linkage
    class NET_DVR_EVENT_CARD_LINKAGE_COND extends Structure {
        public int dwSize;
    ...
    class NET_DVR_EVENT_LINKAGE_INFO extends Structure {
    ...
    class NET_DVR_EVETN_CARD_LINKAGE_UNION extends Structure {
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN];
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("byCardNo", "struEventLinkage", "byMACAddr");
        }
    }

    class NET_DVR_EVENT_CARD_LINKAGE_CFG_V50 extends Structure {
        public int dwSize;
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("dwSize", "byProMode", "byRes1", "dwEventSourceID", "uLinkageInfo", "byAlarmout",
    ...
    class NET_DVR_IPALARMOUTINFO extends Structure {
    ...
    class NET_DVR_IPALARMOUTCFG extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("dwSize", "struIPAlarmOutInfo");
        }
    }
    ...
    class NET_DVR_IPALARMOUTINFO_V40 extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("dwIPID", "dwAlarmOut", "byRes");
        }
    }
    ...
    class NET_DVR_IPALARMOUTCFG_V40 extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("dwSize", "dwCurIPAlarmOutNum", "struIPAlarmOutInfo", "byRes");
        }
    }

    /* IP Alarm input configuration */

    class NET_DVR_IPALARMININFO extends Structure {
        public byte byIPID;                     /* ID of IP device,  the range:  1 to MAX_IP_DEVICE */
    ...
    class NET_DVR_IPALARMININFO_V40 extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("dwIPID", "dwAlarmIn", "byRes");
        }
    }
    ...
    class NET_DVR_IPALARMINCFG_V40 extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("dwSize", "struIPAlarmInInfo", "byRes");
        }
    }
    ...
    class NET_DVR_IPCHANINFO extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("byEnable", "byIPID", "byChannel", "byIPIDHigh", "byTransProtocol", "byres");
        }
    }
    ...
    class NET_DVR_IPSERVER_STREAM extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("byEnable", "byRes", "struIPServer", "wPort", "wDvrNameLen", "byDVRName", "wDVRSerialLen", "byRes1",
    ...
    class NET_DVR_DEV_CHAN_INFO extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("struIP", "wDVRPort", "byChannel", "byTransProtocol", "byTransMode", "byFactoryType", "byDeviceType",
    ...
    class NET_DVR_DDNS_STREAM_CFG extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("byEnable", "byRes1", "NET_DVR_IPADDRstruStreamServer", "wStreamServerPort",
    ...
    class NET_DVR_HKDDNS_STREAM extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("byEnable", "byRes", "byDDNSDomain", "wPort", "wAliasLen", "byAlias", "wDVRSerialLen",
                    "byRes1", "byDVRSerialNumber", "byUserName", "byPassword", "byChannel", "byRes2");
        }
    }

    int NET_DVR_GET_IPPARACFG_V40 = 1062;
    int NET_DVR_SET_IPPARACFG_V40 = 1063;
    ...
    class NET_DVR_GET_STREAM_UNION extends Structure {
    ...
    class NET_DVR_IPPARACFG_V40 extends Structure {
        ...
        @Override
        protected List getFieldOrder() {
            return Arrays.asList("dwSize", "dwGroupNum", "dwAChanNum", "dwDChanNum", "dwStgartDChan", "byAnalogChanEnable",
    ...
    class NET_DVR_ACS_WORK_STATUS_V50 extends Structure {
        public int dwSize;
    ...
    class NET_DVR_ACS_WORK_STATUS extends Structure {
        public int dwSize;
        public byte[] byDoorLockStatus = new byte[MAX_DOOR_NUM_256];//door lock status(relay status), 0 normally closed,1 normally open, 2 damage short - circuit alarm, 3 damage breaking alarm, 4 abnormal alarm
        public byte[] byDoorStatus = new byte[MAX_DOOR_NUM_256];
        public byte[] byMagneticStatus = new byte[MAX_DOOR_NUM_256];
    ...
    class REMOTECONFIGSTATUS_CARD extends Structure {
    ...
    int ENUM_ACS_INTELLIGENT_IDENTITY_DATA = 0x9;
    ...
    interface FMSGCallBack_V31 extends StdCallCallback {
        boolean invoke(NativeLong lCommand, NET_DVR_ALARMER pAlarmer, Pointer pAlarmInfo, int dwBufLen, Pointer pUser);
    }
